...
void simulate()
{
	...
	while(PC!=end)
	{
		...
		//运行
		IF();
		// printf("After Inst Fetch\n");
		ID();
		// printf("After Inst decode\n");
		EX();
		// printf("After exec\n");
		MEM();
		// printf("After mem\n");
		WB();
		// printf("After write back\n");
		inst_num++;
		//更新中间寄存器
		// IF_ID=IF_ID_old;
		// ID_EX=ID_EX_old;
...
void ID()
{
	...
	ID_EX.Reg_Rs = reg[rs1];
	ID_EX.Reg_Rt = reg[rs2];
	printf("Reg_Rt is %d\n", ID_EX.Reg_Rt);
...
void MEM()
{
	...
	case 1: {// read byte
		// mem = memory[ALUout>>2];
		// pos = ALUout % 4;
		ReadOut = *(char*)((unsigned char*)memory + ALUout);
		cycle_num++;
		break;
	...
	case 2:{// load half
		...
		// pos = ALUout % 4;
		ReadOut = *(short*)((unsigned char*)memory + ALUout);
		cycle_num++;
		break;
	}
	case 3:{//load word
		...
		// mem2 = memory[(ALUout>>2) + 1];
		// pos = ALUout % 4;
		ReadOut = *(int*)(  (unsigned char *)memory + ALUout);
		cycle_num++;
		break;
